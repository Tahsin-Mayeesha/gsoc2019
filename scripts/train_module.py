...
print("Getting Data")
with open("data/wiki_articles.pkl",'rb') as f:
    wiki_articles= pickle.load(f)
    
print(len(wiki_articles),"articles")
...
print(len(train_vocab),"words in vocab")

wiki_dataset = tf.data.Dataset.from_tensor_slices(wiki_articles).batch(8,drop_remainder=True)
...
module = ULMFiTModule(vocab=train_vocab,emb_dim=64,buckets=1,state_size=128,n_layers=2)

train_losses = []
step = 0
for epoch in range(1):
    
    for batch in wiki_dataset:
        train_loss = module.train(batch)
        if step%100==0:
            print("Step ",step," Train loss: ",train_loss.numpy())
            # saving model per 100 step
            tf.saved_model.save(module,"language_model")
